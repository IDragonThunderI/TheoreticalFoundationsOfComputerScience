...
int main()
{
    ...
    RAA_Method1();
    cout << "Количество сравнений при заполнении таблицы ID методом бинарного дерева: " << counter1 << endl;
    
    RAA_Method2();
    cout << "Количество сравнений при заполнении таблицы ID методом хэш-функции с упорядоченным списком: " << counter2 << endl;
...
{
    int num = rand() % 52 + 1;
    if (num <= 26) return (64 + num); // Латинские буквы верхнего регистра
    else return (70 + num); // Латинские буквы нижнего регистра
}
...
char RAA_OtherSymbol()
{
    int num = rand() % 63 + 1;
    if (num <= 26) return (64 + num); // Латинские буквы верхнего регистра
    else if (num > 26 && num <= 52) return (70 + num); // Латинские буквы нижнего регистра
    else if (num > 52 && num <= 62) return (num - 5); // Цифры 0-9
    else return (95); // Символ "_"
}
